...
impl LintPass for UnrootedPass {
    ...
    fn check_variant(&mut self, cx: &Context, var: &hir::Variant, _gen: &hir::Generics) {
    ...
    fn check_fn(&mut self, cx: &Context, kind: FnKind, decl: &hir::FnDecl,
                block: &hir::Block, _span: codemap::Span, id: ast::NodeId) {
        match kind {
            FnKind::ItemFn(i, _, _, _, _, _) |
            FnKind::Method(i, _, _) if i.name.as_str() == "new"
                                           || i.name.as_str() == "new_inherited"
                                           || i.name.as_str() == "new_initialized" => {
                self.in_new_function = true;
                return;
            },
            FnKind::ItemFn(_, _, style, _, _, _) => match style {
    ...
    fn check_expr(&mut self, cx: &Context, expr: &hir::Expr) {
        fn require_rooted(cx: &Context, in_new_function: bool, subexpr: &hir::Expr) {
